...
/** @type {Map<string, import('dead-or-alive').Result>} */
    ...
    await pAll(
      urls.map(function (url) {
        return async function () {
          ...
          // Create a cache key that includes the URL and relevant options
          const cacheKey = JSON.stringify({
            url,
            checkAnchor: deadOrAliveOptions.checkAnchor,
            followMetaHttpEquiv: deadOrAliveOptions.followMetaHttpEquiv,
            userAgent: deadOrAliveOptions.userAgent
          })

          // Check if we have a cached result
          let result = urlCache.get(cacheKey)

          if (!result) {
            // If not cached, check the URL and cache the result
            result = await deadOrAlive(url, deadOrAliveOptions)
